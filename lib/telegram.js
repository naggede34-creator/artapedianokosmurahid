...
// Provider badges
const PROVIDER_LABEL = { simuru: "QRIS Simuru", pakasir: "QRIS Pakasir", rumahotp: "QRIS RumahOTP" };
...
export function resellerRegisteredNotif({ token, name, webName, slug, markup }) {
  return (
    `🏪✨ <b>RESELLER BARU DAFTAR!</b> ✨🏪\n` +
    `${HDR}\n\n` +

    `🌐 <b>INFO WEB RESELLER</b>\n` +
    `├ 🏷️ Nama Web  ╸ <b>${esc(webName)}</b>\n` +
    `├ 🔗 Slug URL  ╸ <code>/store/${esc(slug)}</code>\n` +
    `└ 📈 Markup    ╸ <b>+${markup}%</b>\n` +
    `\n` +
    ...
    `🕒 ${nowWIB()}\n` +
    `${HDR}\n` +
    `💼 <i>Web reseller baru aktif & siap terima order!</i>` +
    channelFooter()
  );
}

export function resellerOrderNotif({ resellerWebName, resellerSlug, type, detail, charge, token, balance }) {
  return (
    `🏪💥 <b>ORDER MASUK KE WEB RESELLER!</b>\n` +
    `${HDR}\n\n` +

    `🌐 <b>WEB RESELLER</b>\n` +
    `├ 🏷️ Nama      ╸ <b>${esc(resellerWebName)}</b>\n` +
    `└ 🔗 Slug      ╸ <code>/store/${esc(resellerSlug)}</code>\n` +
    `\n` +
    ...
    `💰 <b>TRANSAKSI</b>\n` +
    `├ 💵 Bayar     ╸ <b>${rupiah(charge)}</b>\n` +
    `└ 🏦 Saldo     ╸ ${rupiah(balance)}\n` +
    `\n` +

    `👤 Pembeli ╸ <code>${maskToken(token)}</code>   🕒 ${nowWIB()}\n` +
    `${HDR}\n` +
    `⚡ <i>Komisi markup langsung masuk ke reseller!</i>` +
    channelFooter()
  );
}
