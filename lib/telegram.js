...
import { CHANNEL_URL, CHANNEL_ID, BOT_URL, WEBAPP_URL, tombolOrder } from "@/lib/links";
...
const PROVIDER_LABEL = {
  warungnokos: "QRIS WarungNokos",
  pakasir: "QRIS Pakasir",
  rumahotp: "QRIS RumahOTP",
  atlantic: "QRIS Atlantic",
  manual: "QRIS Manual (Cek Admin)"
};
...
// Penutup yang sama di semua notif channel: satu ajakan, lalu tautannya.
//
// Ajakannya diputar, bukan satu kalimat yang sama terus. Channel yang mengulang
// kalimat identik ratusan kali sehari berhenti dibaca — mata mengenali
// bentuknya lalu melewatinya, dan ajakan yang dilewati sama saja dengan tidak
// ada.
const AJAKAN = [
  "Nomor OTP mulai Rp2.000 — aktif seketika, tanpa daftar.",
  "Kode tidak masuk? Saldo kembali sendiri. Tanpa komplain, tanpa antre.",
  "Tanpa daftar, tanpa email. Isi saldo, langsung bisa order.",
  "Stok jalan 24 jam. Order jam berapa pun, nomornya keluar seketika.",
  "Deposit QRIS dari e-wallet atau m-banking apa pun, saldo masuk otomatis.",
  "Ratusan layanan, puluhan negara. Cari aplikasinya, langsung order.",
  "Harga termurah dengan refund otomatis — dua-duanya, bukan salah satu."
];
...
function channelFooter() {
  return (
    ...
    `🤖 <a href="${BOT_URL}">Bot</a>  ·  ` +
    `🌐 <a href="${WEBAPP_URL}">Web</a>`
  );
}
...
// Deposit manual: user mengaku sudah bayar, giliran admin memutuskan.
// Ini satu-satunya notif deposit yang MENUNTUT tindakan — makanya nadanya
// berbeda dari notif lain yang cuma mengabarkan.
export function manualDepositReviewNotif({ orderId, amount, token, name, note, hasProof, createdAt }) {
  return (
    `🔔 <b>DEPOSIT MANUAL — PERLU DICEK</b>\n` +
    `${HDR}\n` +
    `💳  <b>${esc(providerLabel("manual"))}</b>\n` +
    `${HDR}\n\n` +
    ...
    `🧾 <b>BUKTI</b>\n` +
    `├ 📎 Lampiran  ╸ ${hasProof ? "ada, lihat di panel admin" : "tidak dilampirkan"}\n` +
    `└ 📝 Catatan   ╸ ${esc(note || "—")}\n` +
    `\n` +

    `⏰ <b>WAKTU</b>\n` +
    `├ 🕒 Dibuat    ╸ ${fmtWIB(createdAt)}\n` +
    `└ 🙋 Konfirmasi╸ ${nowWIB()}\n` +

    `${HDR}\n` +
    `⚠️ <i>Saldo BELUM masuk. Cek mutasi QRIS-mu, lalu setujui atau tolak di panel admin.</i>` +
    channelFooter()
  );
}

// Hasil pengecekan admin atas deposit manual yang ditolak. Yang disetujui
// sudah punya notif sukses sendiri lewat creditDeposit.
export function manualDepositRejectedNotif({ orderId, amount, token, name, reason }) {
  return (
    ...
    `📝 <b>ALASAN</b>\n` +
    `└ ${esc(reason || "Pembayaran tidak ditemukan.")}\n` +
    `\n` +

    `🕒 ${nowWIB()}\n` +
    `${HDR}\n` +
    `💡 <i>Tidak ada saldo yang berubah.</i>` +
    channelFooter()
  );
}
...
// ────────────────────────── PENARIKAN SALDO ──────────────────────────
//
// Uang KELUAR dari saldo Atlantic ke rekening pemilik web. Nomor rekening
// disamarkan seperti nomor lain di notif: yang membaca channel/grup notif
// belum tentu cuma pemiliknya.
export function withdrawNotif({ refId, bankName, accountNumber, ownerName, nominal, fee, total, status, providerId }) {
  const selesai = String(status || "").toLowerCase() === "completed";
  const gagal = ["failed", "canceled"].includes(String(status || "").toLowerCase());
  return (
    `${selesai ? "✅" : gagal ? "❌" : "🟡"} <b>PENARIKAN SALDO ${selesai ? "BERHASIL" : gagal ? "GAGAL" : "DIPROSES"}</b>\n` +
    `${HDR}\n` +
    `🏦  <b>${esc(bankName || "—")}</b>\n` +
    `${HDR}\n\n` +

    `📋 <b>TRANSAKSI</b>\n` +
    `├ 🧾 Ref ID    ╸ <code>${esc(refId)}</code>\n` +
    `└ 🔖 ID Atlantic ╸ <code>${esc(providerId || "—")}</code>\n` +
    `\n` +

    `🏦 <b>REKENING TUJUAN</b>\n` +
    `├ 🔢 Nomor     ╸ <code>${maskPhone(accountNumber)}</code>\n` +
    `└ 📛 Atas nama ╸ ${esc(ownerName || "—")}\n` +
    `\n` +

    `💸 <b>RINCIAN</b>\n` +
    `├ 📦 Nominal   ╸ <b>${rupiah(nominal)}</b>\n` +
    `├ 🧮 Biaya     ╸ ${rupiah(fee || 0)}\n` +
    `└ 💵 Total     ╸ <b>${rupiah(total || nominal)}</b>\n` +
    `\n` +

    `🕒 ${nowWIB()}\n` +
    `${HDR}\n` +
    `🔐 <i>Ditarik dari panel admin.</i>` +
    channelFooter()
  );
}

// Pengumuman pemenang Pembeli Terbanyak mingguan. Tokennya disamarkan seperti
// notif publik lainnya: yang membaca channel bukan cuma pemiliknya.
export function leaderboardWinnersNotif({ winners = [], start, end }) {
  const medali = ["🥇", "🥈", "🥉"];
  const periode = `${fmtWIB(start, { dateOnly: true })} – ${fmtWIB(new Date(new Date(end).getTime() - 86400000), { dateOnly: true })}`;

  return (
    `🏆 <b>PEMENANG PEMBELI TERBANYAK</b>\n` +
    `${HDR}\n` +
    `📅  <b>${esc(periode)}</b>\n` +
    `${HDR}\n\n` +
    winners
      .map(
        (w) =>
          `${medali[w.rank - 1] || `#${w.rank}`} <b>${maskToken(w.token)}</b>\n` +
          `├ 🛒 Transaksi ╸ <b>${w.count}</b>\n` +
          `└ 💰 Hadiah    ╸ <b>${rupiah(w.amount)}</b>`
      )
      .join("\n\n") +
    `\n\n${HDR}\n` +
    `💸 <i>Hadiah sudah masuk saldo otomatis.</i>\n` +
    `🔥 Minggu ini papan peringkat dimulai dari nol — siapa pun bisa naik.` +
    channelFooter()
  );
}
...
export function warrantyResolvedNotif({ approved, orderId, serviceName, countryName, amount, token, adminNote, balance }) {
  return (
    (approved ? `🛡️ <b>KLAIM GARANSI DISETUJUI</b>\n` : `🚫 <b>KLAIM GARANSI DITOLAK</b>\n`) +
    `${HDR}\n\n` +
    ...
    (approved
      ? `💰 <b>PENGEMBALIAN</b>\n` +
        `├ 💵 Nominal   ╸ <b>${rupiah(amount)}</b>\n` +
        `└ 🏦 Saldo kini╸ <b>${rupiah(balance)}</b>\n\n`
      : ``) +

    (adminNote ? `📝 <b>Catatan admin</b>\n${esc(adminNote)}\n\n` : ``) +
...
export function adminPointsAdjustNotif({ token, amount, action, newPoints, note }) {
  const isAdd = action === "add";
  const n = (v) => Number(v || 0).toLocaleString("id-ID");
  return (
    `${isAdd ? "🟢" : "🔴"} <b>POIN ${isAdd ? "DITAMBAH" : "DIKURANGI"} ADMIN</b>\n` +
    `${HDR}\n\n` +

    `⚙️ <b>DETAIL PENYESUAIAN</b>\n` +
    `├ 👤 Akun       ╸ <code>${maskToken(token)}</code>\n` +
    `├ ${isAdd ? "➕" : "➖"} Poin       ╸ <b>${n(amount)}</b>\n` +
    `└ ⭐ Poin Baru  ╸ <b>${n(newPoints)}</b>\n` +
    `\n` +
...
export function depositSuccessPublicNotif({ provider, amount, token, createdAt, depositCount }) {
  ...
  return (
    ...
    `${HDR}\n` +
    `\n<b>💰 ${rupiah(amount)}</b>\n` +
    `<i>Saldo sudah masuk ke akun pembeli</i>\n\n` +
    `💳  Metode      ╸ <b>${esc(providerLabel(provider))}</b>\n` +
    `⚡  Diproses    ╸ ${paidIn ? `<b>${paidIn}</b>` : "<b>otomatis</b>"}\n` +
    `👤  Pembeli     ╸ <code>${maskToken(token)}</code>\n` +
    (cnt > 0 ? `🏆  Deposit ke  ╸ <b>${cnt.toLocaleString("id-ID")}</b>\n` : ``) +
    // QRIS manual memang dicek admin dulu. Menulis "tanpa admin" untuk metode
    // itu bukan cuma salah — ia menjanjikan yang tidak terjadi, lalu pembeli
    // berikutnya memilih manual karena mengira saldonya masuk sendiri.
    (provider === "manual"
      ? `🔎  Konfirmasi  ╸ <b>Dicek admin, lalu masuk</b>\n`
      : `🔓  Konfirmasi  ╸ <b>Tanpa admin, langsung masuk</b>\n`) +
    `\n${HDR_THIN}\n` +
    `🕒 ${nowWIB()}\n` +
    `<i>Saldonya sama untuk bot dan web. Dipakai di mana pun, potongannya satu.</i>` +
    channelFooter()
  );
}
...
export function otpReceivedPublicNotif({ serviceName, countryName, phoneNumber, token, createdAt }) {
  ...
  return (
    ...
    `${HDR}\n` +
    `\n<b>⚡ ${waited || "Seketika"}</b>\n` +
    `<i>Dari nomor dibeli sampai kodenya masuk</i>\n\n` +
    `🏷️  Layanan     ╸ <b>${esc(serviceName || "—")}</b>\n` +
    `🌍  Negara      ╸ ${esc(countryName || "—")}\n` +
    `☎️  Nomor       ╸ <code>${maskPhone(phoneNumber)}</code>\n` +
    `👤  Pembeli     ╸ <code>${maskToken(token)}</code>\n` +
    ...
    `\n${HDR_THIN}\n` +
    `🕒 ${nowWIB()}\n` +
    `<i>Kode OTP tidak pernah ditampilkan di channel — itu milik yang membayarnya.</i>` +
    channelFooter()
  );
}

// ═══════════════════════════════════════════════════════════════════════
// VERSI PUBLIK — untuk channel
//
// Aturan yang berlaku untuk SEMUA template di bawah, tanpa kecuali:
//   - kode akun selalu disamarkan
//   - nomor telepon selalu disamarkan
//   - kode OTP TIDAK PERNAH ikut
//   - saldo, nama asli, dan nomor rekening TIDAK PERNAH ikut
//
// Yang berguna dibaca publik cuma bahwa tokonya hidup dan transaksinya jalan.
// Sisanya milik orangnya sendiri, dan channel bisa dibaca siapa saja.
// ═══════════════════════════════════════════════════════════════════════

/**
 * Bentuk tetap semua notif channel.
 *
 *   ikon + JUDUL
 *   ══════════════
 *   ANGKA UTAMA          <- yang dicari mata pertama kali
 *   keterangan angkanya
 *
 ...
 *   ajakan + tautan
 *
 * `sorot` itu yang membedakan kartu ini dari daftar data: satu angka besar
 * berdiri sendiri di atas. Notif yang semua barisnya sama besar memaksa
 * pembacanya memindai dulu untuk tahu ini soal apa — dan di channel yang
 * ramai, pemindaian itu tidak terjadi; pesannya dilewati.
 */
function kartuPublik(judul, ikon, baris, penutup, sorot) {
  return (
    `${ikon}  <b>${judul}</b>\n` +
    `${HDR}\n` +
    (sorot
      ? `\n<b>${sorot.nilai}</b>\n` + (sorot.label ? `<i>${sorot.label}</i>\n` : "")
      : "") +
    `\n` +
...
const barisPublik = (ikon, label, nilai) => `${ikon}  ${label.padEnd(11, " ")} ╸ ${nilai}`;

export function otpSoldPublicNotif({ serviceName, countryName, phoneNumber, price, operator, token }) {
  return kartuPublik(
    "NOMOR OTP TERJUAL",
    "📱",
    [
      barisPublik("🏷️", "Layanan", `<b>${esc(serviceName || "—")}</b>`),
      barisPublik("🌍", "Negara", esc(countryName || "—")),
      operator ? barisPublik("📶", "Operator", esc(operator)) : null,
      barisPublik("☎️", "Nomor", `<code>${maskPhone(phoneNumber)}</code>`),
      barisPublik("👤", "Pembeli", `<code>${maskToken(token)}</code>`),
      barisPublik("⚡", "Status", "<b>Aktif seketika</b>")
    ],
    "<i>Kode OTP masuk otomatis. Tidak masuk sampai masa aktif habis? Saldo kembali sendiri.</i>",
    { nilai: `💵 ${rupiah(price)}`, label: "Harga yang dibayar pembeli" }
  );
}

export function otpRefundPublicNotif({ serviceName, countryName, price, token, reason }) {
  return kartuPublik(
    ...
    [
      barisPublik("🏷️", "Layanan", `<b>${esc(serviceName || "—")}</b>`),
      barisPublik("🌍", "Negara", esc(countryName || "—")),
      barisPublik("📝", "Sebab", esc(reason || "kode tidak masuk")),
      barisPublik("👤", "Pembeli", `<code>${maskToken(token)}</code>`),
      barisPublik("⏱️", "Diproses", "<b>Otomatis, tanpa diminta</b>")
    ],
    "<i>Tidak perlu komplain, tidak perlu antre. Ini yang membedakan beli di sini.</i>",
    { nilai: `💵 ${rupiah(price)}`, label: "Kembali penuh ke saldo pembeli" }
  );
}

export function transferPublicNotif({ amount, fee = 0 }) {
  return kartuPublik(
    "TRANSFER SALDO ANTAR PENGGUNA",
    "🔁",
    [
      fee > 0 ? barisPublik("🧮", "Biaya", rupiah(fee)) : barisPublik("🧮", "Biaya", "<b>Gratis</b>"),
      barisPublik("⏱️", "Sampai", "<b>Seketika</b>")
    ],
    "<i>Saldo bisa dikirim ke sesama pengguna lewat menu Transfer.</i>",
    { nilai: `💸 ${rupiah(amount)}`, label: "Nominal yang dikirim" }
  );
}

export function voucherPublicNotif({ amount, token }) {
  return kartuPublik(
    "VOUCHER DITEBUS",
    "🎟️",
    [
      barisPublik("👤", "Penebus", `<code>${maskToken(token)}</code>`),
      barisPublik("⚡", "Saldo", "<b>Masuk seketika</b>")
    ],
    "<i>Pantau channel ini — voucher dibagikan sewaktu-waktu, siapa cepat dia dapat.</i>",
    { nilai: `🎁 ${rupiah(amount)}`, label: "Saldo gratis yang didapat" }
  );
}

export function productSoldPublicNotif({ productName, price, category, token }) {
  return kartuPublik(
    "PRODUK DIGITAL TERJUAL",
    "🛍️",
    [
      barisPublik("📦", "Produk", `<b>${esc(productName || "—")}</b>`),
      category ? barisPublik("🗂️", "Kategori", esc(category)) : null,
      barisPublik("👤", "Pembeli", `<code>${maskToken(token)}</code>`),
      barisPublik("⚡", "Pengiriman", "<b>Otomatis setelah dibayar</b>")
    ],
    "<i>Cek menu Produk — stoknya jalan terus.</i>",
    { nilai: `💵 ${rupiah(price)}`, label: "Harga produk" }
  );
}

export function jobApprovedPublicNotif({ jobTitle, reward, token }) {
  return kartuPublik(
    "SALDO GRATIS CAIR",
    "💼",
    [
      barisPublik("🗒️", "Tugas", `<b>${esc(jobTitle || "—")}</b>`),
      barisPublik("👤", "Penerima", `<code>${maskToken(token)}</code>`),
      barisPublik("✅", "Status", "<b>Sudah dicek admin</b>")
    ],
    "<i>Belum punya saldo? Kerjakan tugas di menu Saldo Gratis, modalnya nol.</i>",
    { nilai: `💰 ${rupiah(reward)}`, label: "Imbalan yang cair" }
  );
}

export function pointsPublicNotif({ points, rupiah: rp, token }) {
  return kartuPublik(
    "POIN DITUKAR JADI SALDO",
    "⭐",
    [
      barisPublik("🎯", "Poin", `<b>${Number(points || 0).toLocaleString("id-ID")}</b>`),
      barisPublik("👤", "Penukar", `<code>${maskToken(token)}</code>`)
    ],
    "<i>Tiap transaksi sukses menambah poin. Makin sering order, makin banyak baliknya.</i>",
    { nilai: `💰 ${rupiah(rp)}`, label: "Saldo yang didapat dari poin" }
  );
}

export function warrantyPublicNotif({ serviceName, amount, token }) {
  return kartuPublik(
    "KLAIM GARANSI DISETUJUI",
    "🛡️",
    [
      serviceName ? barisPublik("🏷️", "Layanan", `<b>${esc(serviceName)}</b>`) : null,
      barisPublik("👤", "Pemilik", `<code>${maskToken(token)}</code>`),
      barisPublik("✅", "Status", "<b>Disetujui, saldo sudah masuk</b>")
    ],
    "<i>Nomor bermasalah? Ajukan garansi lewat dashboard — dicek manual oleh admin.</i>",
    { nilai: `💵 ${rupiah(amount)}`, label: "Dikembalikan penuh" }
  );
}

export function petLevelPublicNotif({ level, tahapNama, bonus, token }) {
  return kartuPublik(
    "PET ARTA PEDIA NAIK LEVEL",
    "🦅",
    [
      barisPublik("🐣", "Tahap", `<b>${esc(tahapNama || "—")}</b>`),
      barisPublik("🎁", "Bonus", `<b>+${bonus}%</b> cashback tiap deposit`),
      barisPublik("👤", "Pemilik", `<code>${maskToken(token)}</code>`)
    ],
    "<i>Rawat elangmu tiap hari di menu Pet — tiap level menambah cashback, selamanya.</i>",
    { nilai: `📈 LEVEL ${level}`, label: "Level baru" }
  );
}

export function depositPendingPublicNotif({ provider, amount, expiredAt }) {
  const sisa = expiredAt ? Math.max(0, Math.round((new Date(expiredAt).getTime() - Date.now()) / 60000)) : null;
  return kartuPublik(
    "DEPOSIT MASUK ANTREAN",
    "🟡",
    [
      barisPublik("💳", "Metode", `<b>${esc(providerLabel(provider))}</b>`),
      sisa !== null ? barisPublik("⏳", "Berlaku", `${sisa} menit lagi`) : null,
      barisPublik("⚡", "Saldo", "<b>Masuk otomatis setelah dibayar</b>")
    ],
    "<i>Isi saldo pakai QRIS dari e-wallet atau m-banking apa pun.</i>",
    { nilai: `💰 ${rupiah(amount)}`, label: "Menunggu dibayar" }
  );
}

export function depositCanceledPublicNotif({ provider, amount, reason }) {
  const kedaluwarsa = reason === "expired";
  return kartuPublik(
    kedaluwarsa ? "QRIS KEDALUWARSA" : "DEPOSIT DIBATALKAN",
    kedaluwarsa ? "⏰" : "❌",
    [
      barisPublik("💳", "Metode", esc(providerLabel(provider))),
      barisPublik("💰", "Nominal", rupiah(amount)),
      barisPublik("🔒", "Saldo", "<b>Tidak ada yang terpotong</b>")
    ],
    "<i>Buat QRIS baru kapan saja, tidak ada batasnya.</i>"
  );
}
  ...
  try {
    const res = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
      ...
      body: JSON.stringify({
        ...
        reply_markup: { inline_keyboard: tombolOrder() }
      })
    });
    if (!res.ok) {
 ...
 */
/**
 * Channel tujuan, menurut urutan ini:
 *   1. pengaturan panel admin (kalau diisi)
 *   2. env TELEGRAM_CHANNEL_ID
 *   3. channel resmi di lib/links.js
 *
 * Nomor 3 ada supaya notif tetap sampai walau dua yang di atas belum diisi
 * sama sekali. Yang perlu diingat: dua yang di atas MENIMPA-nya, jadi kalau
 * notif muncul di channel lama, isi kolom di panel admin (atau env) itu yang
 * perlu dibersihkan — mengganti berkas links.js saja tidak akan berpengaruh.
 */
export function channelTujuan(settings) {
  const dariPanel = settings?.telegramChannelId;
  if (dariPanel) return String(dariPanel).trim();
  if (process.env.TELEGRAM_CHANNEL_ID) return String(process.env.TELEGRAM_CHANNEL_ID).trim();
  return CHANNEL_ID;
}
...
export async function sendTelegramChannelNotif(text, { pin = false } = {}) {
  ...
  const channelId = channelTujuan(settings);
...
export async function sendTelegramPhoto(pngBuffer, caption) {
  ...
  try {
    ...
    form.append(
      "reply_markup",
      JSON.stringify({ inline_keyboard: tombolOrder() })
    );
