...
function kartuPublik(judul, ikon, baris, penutup) {
  return (
    `${ikon}  <b>${judul}</b>\n` +
    `${HDR}\n\n` +
...
const barisPublik = (ikon, label, nilai) => `${ikon}  ${label.padEnd(9, " ")} ╸ ${nilai}`;

export function otpSoldPublicNotif({ serviceName, countryName, phoneNumber, price, operator, token }) {
  return kartuPublik("NOMOR OTP TERJUAL", "📱", [
    ...
    barisPublik("💵", "Harga", `<b>${rupiah(price)}</b>`),
    barisPublik("👤", "Pembeli", `<code>${maskToken(token)}</code>`)
  ], "<i>Nomor aktif seketika — kode OTP masuk otomatis.</i>");
}

export function otpRefundPublicNotif({ serviceName, countryName, price, token, reason }) {
  return kartuPublik("SALDO DIKEMBALIKAN", "↩️", [
    barisPublik("🏷️", "Layanan", `<b>${esc(serviceName || "—")}</b>`),
    barisPublik("🌍", "Negara", esc(countryName || "—")),
    barisPublik("💵", "Refund", `<b>${rupiah(price)}</b>`),
    barisPublik("👤", "Pembeli", `<code>${maskToken(token)}</code>`),
    barisPublik("📝", "Sebab", esc(reason || "kode tidak masuk"))
  ], "<i>Kode tidak masuk? Saldo kembali sendiri. Tanpa komplain, tanpa antre.</i>");
}

export function transferPublicNotif({ amount, fee = 0 }) {
  return kartuPublik("TRANSFER SALDO ANTAR PENGGUNA", "🔁", [
    barisPublik("💸", "Nominal", `<b>${rupiah(amount)}</b>`),
    fee > 0 ? barisPublik("🧮", "Biaya", rupiah(fee)) : null
  ], "<i>Saldo bisa dikirim ke sesama pengguna, sampai seketika.</i>");
}

export function voucherPublicNotif({ amount, token }) {
  return kartuPublik("VOUCHER DITEBUS", "🎟️", [
    barisPublik("💰", "Nilai", `<b>${rupiah(amount)}</b>`),
    barisPublik("👤", "Penebus", `<code>${maskToken(token)}</code>`)
  ], "<i>Pantau channel ini — voucher dibagikan sewaktu-waktu.</i>");
}

export function productSoldPublicNotif({ productName, price, category, token }) {
  return kartuPublik("PRODUK DIGITAL TERJUAL", "🛍️", [
    barisPublik("📦", "Produk", `<b>${esc(productName || "—")}</b>`),
    category ? barisPublik("🗂️", "Kategori", esc(category)) : null,
    barisPublik("💵", "Harga", `<b>${rupiah(price)}</b>`),
    barisPublik("👤", "Pembeli", `<code>${maskToken(token)}</code>`)
  ], "<i>Dikirim otomatis setelah dibayar.</i>");
}

export function jobApprovedPublicNotif({ jobTitle, reward, token }) {
  return kartuPublik("SALDO GRATIS CAIR", "💼", [
    barisPublik("🗒️", "Tugas", `<b>${esc(jobTitle || "—")}</b>`),
    barisPublik("💰", "Imbalan", `<b>${rupiah(reward)}</b>`),
    barisPublik("👤", "Penerima", `<code>${maskToken(token)}</code>`)
  ], "<i>Kerjakan tugas di menu Saldo Gratis, cair setelah dicek admin.</i>");
}

export function pointsPublicNotif({ points, rupiah: rp, token }) {
  return kartuPublik("POIN DITUKAR JADI SALDO", "⭐", [
    barisPublik("🎯", "Poin", `<b>${Number(points || 0).toLocaleString("id-ID")}</b>`),
    barisPublik("💰", "Jadi", `<b>${rupiah(rp)}</b>`),
    barisPublik("👤", "Penukar", `<code>${maskToken(token)}</code>`)
  ], "<i>Tiap transaksi sukses menambah poin. Tukar kapan saja.</i>");
}

export function warrantyPublicNotif({ serviceName, amount, token }) {
  return kartuPublik("KLAIM GARANSI DISETUJUI", "🛡️", [
    serviceName ? barisPublik("🏷️", "Layanan", `<b>${esc(serviceName)}</b>`) : null,
    barisPublik("💵", "Dikembalikan", `<b>${rupiah(amount)}</b>`),
    barisPublik("👤", "Pemilik", `<code>${maskToken(token)}</code>`)
  ], "<i>Nomor bermasalah? Ajukan garansi lewat dashboard.</i>");
}

export function petLevelPublicNotif({ level, tahapNama, bonus, token }) {
  return kartuPublik("PET NAIK LEVEL", "🥚", [
    barisPublik("🦅", "Tahap", `<b>${esc(tahapNama || "—")}</b>`),
    barisPublik("📈", "Level", `<b>${level}</b>`),
    barisPublik("🎁", "Bonus", `<b>+${bonus}%</b> cashback deposit`),
    barisPublik("👤", "Pemilik", `<code>${maskToken(token)}</code>`)
  ], "<i>Rawat petmu tiap hari — bonusnya naik terus.</i>");
}

export function depositCanceledPublicNotif({ provider, amount, reason }) {
  const kedaluwarsa = reason === "expired";
  return kartuPublik(kedaluwarsa ? "QRIS KEDALUWARSA" : "DEPOSIT DIBATALKAN", kedaluwarsa ? "⏰" : "❌", [
    barisPublik("💳", "Metode", esc(providerLabel(provider))),
    barisPublik("💰", "Nominal", rupiah(amount))
  ], "<i>Tidak ada saldo yang terpotong. Buat QRIS baru kapan saja.</i>");
}
