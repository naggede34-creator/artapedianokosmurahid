...
export function productCreatedNotif({ name, price, category, stock, deliveryType, description }) {
  ...
  return (
    ...
    `├ 💰 Harga       : <b>${rupiah(price)}</b>\n` +
    `├ 📦 Stok        : <b>${stockText}</b>\n` +
    `└ 🚚 Pengiriman  : ${delivLabel}\n` +
    (description
      ? `\n📝 <b>Deskripsi</b>\n└ <i>${esc(String(description).slice(0, 150))}${description.length > 150 ? "…" : ""}</i>\n`
      : "") +
    `\n` +
    `${DIVIDER}\n` +
...
export function productBoughtNotif({ productName, price, category, deliveryType, token, name, balance, ref }) {
  ...
  return (
    ...
    `├ 🔑 Token       : <code>${maskToken(token)}</code>\n` +
    (name ? `├ 📛 Nama        : ${esc(name)}\n` : "") +
    `└ 🏦 Saldo sisa  : <b>${rupiah(balance)}</b>\n` +
    `\n` +
    `🧾 <b>Ref Order</b>  : <code>${esc(ref)}</code>\n` +
    `🕒 <b>Waktu</b>      : ${nowWIB()}\n` +
    `${DIVIDER}\n` +
...
export function jobCreatedNotif({ title, reward, category, maxCompletions, proofType, description }) {
  ...
  return (
    ...
    `└ 📎 Bukti       : ${proofLabel}\n` +
    (description
      ? `\n📝 <b>Deskripsi</b>\n└ <i>${esc(String(description).slice(0, 180))}${description.length > 180 ? "…" : ""}</i>\n`
      : "") +
    `\n` +
    `${DIVIDER}\n` +
    `🙋 <i>User bisa langsung mengambil job ini sekarang!</i>` +
    channelFooter()
  );
}
...
export function jobApprovedNotif({ jobTitle, reward, token, name, balance, completedCount, maxCompletions, submittedAt }) {
  ...
  return (
    `✅🎉 <b>JOB DISELESAIKAN &amp; DISETUJUI!</b> 🎉✅\n` +
    `${DIVIDER}\n` +
    `\n` +
    `💼 <b>Detail Job</b>\n` +
    `├ 📌 Judul       : <b>${esc(jobTitle)}</b>\n` +
    `├ 🎁 Reward      : <b>${rupiah(reward)}</b>\n` +
    `└ 📊 Selesai ke  : <b>#${Number(completedCount).toLocaleString("id-ID")}</b>${slotLeft !== null ? ` (sisa ${slotLeft} slot)` : ""}\n` +
    `\n` +
    `👤 <b>Peserta</b>\n` +
    `├ 🔑 Token       : <code>${maskToken(token)}</code>\n` +
    (name ? `├ 📛 Nama        : ${esc(name)}\n` : "") +
