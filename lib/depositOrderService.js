...
import { getSettings, depositLimits, manualDepositReady } from "@/lib/settings";
...
export async function createDepositForToken({ token, amount, provider }) {
  ...
  if (chosen === MANUAL_DEPOSIT_KEY && !manualDepositReady(settingsDoc)) {
  ...
  if (chosen === WARUNGNOKOS_DEPOSIT_KEY) {
    ...
    try {
      ...
      return fail(400, err?.message || "Gagal membuat QRIS Atlantic, coba metode lain.");
    }
    providerRef = String(dep.id || "");
...
export async function confirmManualDeposit({ token, orderId, proofImage, note }) {
  ...
  if (proof && !proof.startsWith("data:image/")) return fail(400, "Bukti bayar harus berupa gambar.");
