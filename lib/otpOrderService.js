...
// Harga RuangOTP selalu diambil ulang dari provider saat order: selain mencegah
// harga dimanipulasi dari browser, expected_price yang dikirim ke RuangOTP wajib
// sama persis dengan pricelist mereka saat itu.
async function resolveRuangOtp(serverId, serviceId, countryId, providerKey) {
  const rows = await getRuangOtpCountries(serverId, serviceId);
...
export async function placeOtpOrder(input) {
  ...
  const isRo = isRuangOtpServer(server);
  ...
  try {
    ...
    if (!isRo && !isBn && (!numberId || !providerId)) {
      return { ok: false, status: 400, error: "Parameter kurang. Muat ulang halaman lalu coba lagi." };
    }
    if ((isRo || isBn) && (countryId === undefined || countryId === null || countryId === "")) {
    ...
    if (list.find((s) => s.id === serverId)?.enabled === false) {
      return { ok: false, status: 503, error: "Server ini sedang dinonaktifkan admin." };
    }

    const resolved = isBn
      ? await resolveDibanana(serviceId, countryId, Number(providerIndex) || 0)
      : isRo
      ? await resolveRuangOtp(serverId, serviceId, countryId, providerId)
    ...
    if (isBn) {
      ...
      let failMsg = null;
      try {
        data = await createRuangOtpOrder(serverId, {
          key: resolved.providerKey,
          operator: operatorId || "any",
          // Harga modal apa adanya — RuangOTP menolak kalau tidak sama persis.
          expectedPrice: resolved.price
        });
      } catch (e) {
        failMsg = e?.message || null;
        console.error("[otpOrder] RuangOTP buy gagal:", failMsg);
      }
      if (!data || !data.id) {
        await users.updateOne({ token }, { $inc: { balance: sellPrice } });
        debited = false;
      ...
      // RuangOTP tidak mengirim waktu kedaluwarsa; masa aktif standarnya 20 menit.
    ...
    await orders.insertOne({
      orderId,
      token,
      server: isRo || isBn ? server : "rumahotp",
    ...
    if (notify) {
      sendTelegramNotif(
        otpPurchaseNotif({
          ...
          token,
          name: user.name,
          operator: isRo || isBn ? operatorId || "any" : operatorName,
