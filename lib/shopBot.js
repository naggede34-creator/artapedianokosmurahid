...
//
// Bot toko bisa lebih dari satu. Bot PERTAMA tokennya dari environment
// SHOP_BOT_TOKEN; bot tambahan didaftarkan admin lewat dasbor dan tokennya
// disimpan di koleksi `bots`. Keduanya dilayani kode yang sama — bot mana yang
// menjawab ditentukan konteks permintaan (lib/botContext.js).
//
// Token bot tidak boleh pernah ditulis langsung di kode atau dikirim ke
// browser — siapa pun yang memegangnya bisa mengambil alih bot dan membaca
// seluruh percakapan pembeli.
import { botSessionsCol } from "@/lib/db";
import { botAktif } from "@/lib/botContext";

const API = "https://api.telegram.org";

// Token bot yang sedang melayani permintaan ini.
//
// Kalau ada bot aktif di konteks (permintaan datang lewat webhook bot yang
// ditambahkan admin), tokennya yang dipakai. Di luar itu, bot pertama dari
// environment. Urutan ini penting: membalikkannya berarti bot kedua menjawab
// memakai token bot pertama, dan Telegram membuang pesannya karena pembelinya
// tidak pernah memulai bot pertama.
...
/** Id bot yang sedang melayani. "env" untuk bot pertama dari environment. */
export function shopBotId() {
  const bot = botAktif();
  return bot?.botId ? String(bot.botId) : "env";
}
...
// Sesi dikunci per (bot, chat), bukan per chat saja.
//
// Id chat Telegram melekat pada ORANGNYA, bukan pada botnya: orang yang sama
// punya id chat yang sama persis di bot A dan bot B. Dengan kunci chatId saja,
// langkah yang sedang berjalan di bot A terbaca oleh bot B — dan karena sesi
// ini menyimpan kode akun yang tertaut serta deposit yang sedang berjalan,
// yang bocor bukan cuma tampilan menu.
//
// Dokumen lama (sebelum ada bot kedua) tidak punya botId. Dokumen itu dibaca
// sebagai milik bot pertama, supaya tidak ada yang tiba-tiba ter-logout.
function kunciSesi(chatId) {
  const bot = shopBotId();
  return bot === "env"
    ? { chatId: String(chatId), botId: { $in: [null, "env"] } }
    : { chatId: String(chatId), botId: bot };
}

export async function getSession(chatId) {
  const col = await botSessionsCol();
  return (await col.findOne(kunciSesi(chatId))) || { chatId: String(chatId), botId: shopBotId() };
}
...
export async function setSession(chatId, patch) {
  ...
  await col.updateOne(
    kunciSesi(chatId),
    { $set: { ...patch, chatId: String(chatId), botId: shopBotId(), updatedAt: new Date() } },
    { upsert: true }
  );
}
...
export async function clearStep(chatId) {
  const col = await botSessionsCol();
  await col.updateOne(kunciSesi(chatId), { $unset: { step: "" } });
}
...
import { usersCol, botsCol } from "@/lib/db";
import { jalankanDenganBot } from "@/lib/botContext";

// Dibaca langsung dari koleksinya, bukan lewat lib/bots.js: lib/bots.js
// mengimpor lib/indexes.js, dan berkas ini dipakai dari alur pesanan dan
// deposit — lingkaran impor di jalur uang tidak sepadan dengan
// kenyamanan satu fungsi.
async function ambilBotTerdaftar(botId) {
  try {
    const col = await botsCol();
    return await col.findOne({ botId: String(botId) });
  } catch {
    return null;
  }
}

async function tautanTelegram(token) {
  try {
    const users = await usersCol();
    const u = await users.findOne({ token }, { projection: { telegramChatId: 1, telegramBotId: 1 } });
    if (!u?.telegramChatId) return null;
    return { chatId: u.telegramChatId, botId: u.telegramBotId || "env" };
  } catch {
    return null;
  }
}
...
/**
 * Notifikasi dari alur WEB ke pembeli di bot.
 *
 * Dipanggil di luar webhook, jadi tidak ada bot aktif di konteks. Bot yang
 * dipakai harus bot yang BENAR-BENAR ditautkan orang itu: id chat Telegram
 * melekat pada orangnya dan sama di semua bot, jadi mengirim lewat bot yang
 * salah menghasilkan "bot can't initiate conversation" — pesannya hilang
 * tanpa ada yang tahu, justru di kabar yang paling ditunggu (kode OTP masuk,
 * deposit berhasil).
 */
export async function notifyBotUser(token, text, keyboard) {
  const taut = await tautanTelegram(token);
  if (!taut) return;

  if (taut.botId && taut.botId !== "env") {
    const bot = await ambilBotTerdaftar(taut.botId);
    if (!bot?.token || bot.aktif === false) {
      // Botnya dihapus atau dimatikan admin. Tidak dialihkan ke bot lain:
      // orang ini tidak pernah memulai bot lain, jadi pesannya tetap tidak
      // akan sampai — dan mencobanya hanya menyembunyikan sebabnya dari log.
      console.warn(`[shopBot] notif untuk ${token} dilewati: bot ${taut.botId} tidak aktif.`);
      return;
    }
    try {
      await jalankanDenganBot(bot, () => sendMessage(taut.chatId, text, keyboard));
    } catch (err) {
  ...
  if (!(process.env.SHOP_BOT_TOKEN || "").trim()) return;
  try {
    await sendMessage(taut.chatId, text, keyboard);
  } catch (err) {
    console.error("[shopBot] notif user gagal:", err?.message || err);
  }
}
