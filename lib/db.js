...
// Reseller web nokos — user yang beli paket reseller & konfigurasi toko mereka.
export async function resellersCol() {
  return (await getDb()).collection("resellers");
}

// Transaksi dari toko reseller (OTP/SMM yang dipesan via web reseller).
export async function resellerOrdersCol() {
  return (await getDb()).collection("reseller_orders");
}
