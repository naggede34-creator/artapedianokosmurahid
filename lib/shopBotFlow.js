...
import { WEBAPP_URL, CHANNEL_URL, CHANNEL_LIST_URL } from "@/lib/links";
...
// Tujuan SEMUA tombol "buka di web" di bot ini.
//
// Ini Mini App Telegram, bukan alamat situsnya. Bedanya penting: Mini App
// terbuka DI DALAM Telegram dengan akun yang sudah tertaut, sedangkan alamat
// situs melemparkan orang keluar ke peramban, tempat mereka harus memasukkan
// kode akunnya lagi. Yang kedua itu tempat orang berhenti.
//
// Alamatnya diambil dari lib/links.js supaya tidak ada dua salinan yang bisa
// berbeda — sebelumnya menu utama memakai Mini App sementara layar Akun dan
// Informasi memakai siteUrl(), jadi dua tombol yang namanya sama membawa orang
// ke dua tempat berbeda.
...
function menuKeyboard(settings, user, { forOwner = false } = {}) {
  ...
  // Dua tautan keluar ditaruh paling bawah, berdampingan: keduanya membawa
  // orang PERGI dari menu ini, jadi mereka tidak boleh berdiri di antara
  // tombol-tombol yang justru dimaksudkan untuk ditekan di sini.
  rows.push([
    { text: "🌐 Buka di Web", url: WEBAPP_URL },
    { text: "📢 Channel Info", url: CHANNEL_LIST_URL }
  ]);
...
export async function doLogin(chatId, code) {
  ...
  await users.updateOne(
    { token: user.token },
    // telegramBotId ikut dicatat: id chat Telegram sama di semua bot, jadi
    // tanpa ini notifikasi dari web akan dikirim lewat bot pertama ke orang
    // yang hanya pernah memulai bot kedua — dan Telegram membuangnya.
    { $set: { telegramChatId: String(chatId), telegramBotId: shopBotId() } }
  );
  await clearStep(chatId);
...
// Statistik dan broadcast dibatasi pada BOT YANG SEDANG DIPAKAI.
//
// Bot tidak bisa mengirim pesan ke orang yang belum pernah memulai bot itu.
// Jadi menyiarkan dari bot B ke sesi milik bot A bukan cuma sia-sia — setiap
// kirimannya gagal, lalu laporannya berbunyi "gagal: 4.000" dan admin
// menyimpulkan broadcast-nya rusak.
//
// Dokumen lama (sebelum ada bot kedua) belum punya botId; dokumen itu milik
// bot pertama.
function saringBot() {
  const id = shopBotId();
  return id === "env" ? { botId: { $in: [null, "env"] } } : { botId: id };
}
...
export async function showAdminPanel(chatId, messageId) {
  ...
  const saring = saringBot();
  const [totalUser, totalOrder, botUser, botLinked] = await Promise.all([
    users.countDocuments(),
    orders.countDocuments(),
    sessions.countDocuments(saring),
    sessions.countDocuments({ ...saring, token: { $ne: null } })
  ]);
  ...
  return editMessage(
    ...
    `🛠  <b>PANEL ADMIN BOT</b>\n${RULE}\n\n` +
      `👥 User web      ╸ ${totalUser}\n` +
      `🤖 User bot ini  ╸ ${botUser}\n` +
      `🔗 Sudah tertaut ╸ ${botLinked}\n` +
      `📦 Total pesanan ╸ ${totalOrder}\n\n` +
      `<i>Angka user bot di atas hanya untuk bot ini. Bot lain punya hitungannya sendiri.</i>\n\n` +
      `Kirim <code>/broadcast pesan</code> untuk menyiarkan ke user bot ini.`,
    [
...
export async function doBroadcast(chatId, text) {
  ...
  const sessions = await (await import("@/lib/db")).botSessionsCol();
  const all = await sessions.find(saringBot(), { projection: { chatId: 1 } }).toArray();
