...
import { getSettings, serverDisplay, serverOfflineMessage, depositDisplay, markupForServer, manualDepositReady } from "@/lib/settings";
...
export async function showWelcome(chatId, from = null) {
  ...
  const caption =
    ...
    `\n` +
    (user
      ? row("👤", `<code>${esc(user.token)}</code>`) +
        row("💰", `<b>${rupiah(user.balance)}</b>`) +
    ...
    `\n${RULE_THIN}\n` +
    `⚡ ${aktif} server aktif  ·  🕐 24 jam  ·  🔁 Refund otomatis`;
...
export async function showAccount(chatId, messageId) {
  ...
  return editMessage(
    ...
    head("👤", "AKUN KAMU") +
      ...
      row("🔑", `<code>${esc(user.token)}</code>`) +
      row("💰", `<b>${rupiah(user.balance)}</b>`) +
      row("📦", `${total} pesanan · ${sukses} sukses`) +
      `\n${RULE_THIN}\n` +
      `<i>Kode akun ini juga dipakai untuk masuk di web. Saldonya satu, tidak terpisah.</i>`,
    keyboard
  );
}
...
export async function confirmOrder(chatId, messageId, coIdx, prIdx) {
  ...
  return editMessage(
    ...
    head("🧾", "KONFIRMASI PESANAN", `${esc(session.service?.n || "-")} · ${c.f ? `${c.f} ` : ""}${esc(c.n)}`) +
      ...
      row("🖥", esc(d.name)) +
      row("📦", esc(price.nm)) +
      row("💵", `<b>${rupiah(price.s)}</b>`) +
      `\n${RULE_THIN}\n` +
      row("💰", `Saldo sekarang · ${rupiah(user.balance)}`) +
      (kurang
        ? `\n⚠️ <b>Kurang ${rupiah(price.s - user.balance)}.</b> Isi saldo dulu ya.`
        : row("💳", `Sisa setelah beli · <b>${rupiah(user.balance - price.s)}</b>`)),
    kurang
      ? [
          [{ text: "💰 Isi Saldo", callback_data: "dep" }],
...
function orderText(order, serviceName, countryName, statusLabel, otp = null) {
  return (
    ...
    (otp
      ...
      : "") +
    row("☎️", `<code>${esc(order.phoneNumber)}</code>`) +
    row("💵", rupiah(order.price)) +
    (otp ? "" : row("⏳", `<b>${esc(statusLabel)}</b>`)) +
    `\n${RULE_THIN}\n` +
    (otp
      ? `🆔 <code>${esc(order.orderId)}</code>  ·  💰 <b>${rupiah(order.balance)}</b>`
      : `Ketuk nomor untuk menyalin, lalu pakai di aplikasi tujuan.\n` +
        `🆔 <code>${esc(order.orderId)}</code>  ·  💰 <b>${rupiah(order.balance)}</b>`)
  );
}
...
export async function askCancelOrder(chatId, messageId, orderId) {
  ...
  return editMessage(
    ...
    head("❌", "BATALKAN PESANAN?", esc(order.serviceName || "-")) +
      `\n` +
      row("☎️", `<code>${esc(order.phoneNumber)}</code>`) +
      row("💵", `Refund · <b>${rupiah(order.price)}</b>`) +
      `\n${RULE_THIN}\n` +
...
export async function doCancelOrder(chatId, messageId, orderId) {
  ...
  return editMessage(
    ...
    head("✅", "PESANAN DIBATALKAN") +
      `\n` +
      (r.refundAmount ? row("💵", `Saldo kembali · <b>${rupiah(r.refundAmount)}</b>`) : "") +
      row("💰", `Saldo sekarang · <b>${rupiah(segar?.balance)}</b>`) +
      `\n${RULE_THIN}\n` +
...
export async function createDeposit(chatId, amount, messageId = null) {
  ...
  const caption =
    ...
    row("💵", `Saldo masuk · <b>${rupiah(amount)}</b>`) +
    (data.adminFee ? row("🧾", `Biaya admin · ${rupiah(data.adminFee)}`) : "") +
    (manual && data.manualInfo?.accountName ? row("🏷", `Atas nama · <b>${esc(data.manualInfo.accountName)}</b>`) : "") +
    `\n${RULE_THIN}\n` +
...
// "Saya sudah bayar" untuk deposit manual. Lewat bot tidak ada unggahan bukti —
// mengirim foto di Telegram butuh langkah percakapan sendiri, sementara yang
// mendesak adalah admin TAHU ada yang menunggu. Adminnya melihat transaksi ini
// bertanda "tanpa bukti" dan tetap bisa mencocokkannya lewat nominal dan jam.
export async function confirmDepositManual(chatId, messageId, orderId) {
  ...
  const r = await confirmManualDeposit({ token: user.token, orderId, note: "Dikonfirmasi lewat bot Telegram" });
  if (!r.ok) {
  ...
  if (r.status !== "review") {
    // Sudah tidak menunggu lagi — biar layar status yang menjelaskan keadaannya.
  ...
  return sendMessage(
    ...
    `🔎  <b>KONFIRMASI TERKIRIM</b>\n${RULE_THIN}\n\n` +
      `🆔 <code>${esc(orderId)}</code>\n\n` +
      `Admin sedang mencocokkan pembayaranmu dengan mutasi QRIS, biasanya 5–15 menit.\n` +
