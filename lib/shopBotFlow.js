...
export async function askCancelOrder(chatId, messageId, orderId) {
  ...
  if (sisa > 0) {
    return editMessage(
      chatId,
      messageId,
      `⏳ <b>Belum Bisa Dibatalkan</b>\n━━━━━━━━━━━━━━━━━━━━\n\n` +
        `Pesanan baru bisa dibatalkan <b>3 menit</b> setelah dibeli.\n` +
        `Sisa waktu tunggu ╸ <b>${Math.ceil(sisa / 1000)} detik</b>.\n\n` +
        `Jeda ini dari sisi penyedia nomor — seringnya OTP justru masuk di menit-menit ini.`,
      [
  ...
  return editMessage(
    ...
    `❌ <b>Batalkan Pesanan?</b>\n━━━━━━━━━━━━━━━━━━━━\n\n` +
      `📱 Aplikasi ╸ <b>${esc(order.serviceName || "-")}</b>\n` +
      `☎️ Nomor    ╸ <code>${esc(order.phoneNumber)}</code>\n` +
      `💵 Refund   ╸ <b>${rupiah(order.price)}</b>\n\n` +
...
export async function doCancelOrder(chatId, messageId, orderId) {
  ...
  return editMessage(
    ...
    `✅ <b>Pesanan Dibatalkan</b>\n━━━━━━━━━━━━━━━━━━━━\n\n` +
      ...
      (r.refundAmount ? `💵 Saldo kembali ╸ <b>${rupiah(r.refundAmount)}</b>\n` : "") +
      `💰 Saldo sekarang ╸ <b>${rupiah(segar?.balance)}</b>\n\n` +
      (r.message ? `<i>${esc(r.message)}</i>\n\n` : "") +
      `Saldo ini juga langsung terpakai di website.`,
    [
