...
//   1. menanyakan status deposit ke provider (OTPMANIA / Pakasir / RumahOTP), dan
...
import { checkTransaction, normalizePakasirTransaction } from "@/lib/pakasir";
import { checkDeposit } from "@/lib/rumahotp";
import { getOtpmaniaDepositStatus } from "@/lib/otpmania";
...
export async function fetchProviderStatus(deposit) {
  try {
    if (deposit.provider === "otpmania") {
      const data = await getOtpmaniaDepositStatus(deposit.providerRef || deposit.orderId);
      return data.status;
    }
...
export async function creditDeposit(deposit) {
  ...
  await logBalance({
    ...
    title: `Deposit ${deposit.provider === "otpmania" ? "QRIS OTPMANIA" : deposit.provider === "rumahotp" ? "QRIS RumahOTP" : "QRIS Pakasir"}`,
    ref: deposit.orderId
  });
