...
import { sendTelegramNotif, sendTelegramPhoto, depositSuccessNotif, depositCanceledNotif } from "@/lib/telegram";
import { generateReceiptPng, depositSuccessParams } from "@/lib/receiptImage";
...
export async function creditDeposit(deposit) {
  ...
  generateReceiptPng(depositSuccessParams({
    orderId: deposit.orderId,
    token: deposit.token,
    name: finalUser?.name,
    provider: deposit.provider,
    ...
    balance: finalUser?.balance ?? 0,
    depositCount: finalUser?.depositCount
  })).then((png) => sendTelegramPhoto(png, successText.slice(0, 800))).catch((err) => console.error("[receipt/deposit-success]", err?.message || err));
