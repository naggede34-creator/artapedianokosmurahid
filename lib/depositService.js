...
export async function fetchProviderStatus(deposit) {
  try {
    ...
    if (deposit.provider === "rumahotp") {
      ...
      const result = await checkDeposit(rumahOtp, deposit.providerRef || deposit.orderId);
...
export async function creditDeposit(deposit) {
  ...
  if (updatedUser.referredBy) {
    ...
    if (firstTime) {
      const { referralBonusPercent } = await getApiKeys();
      const percent = referralBonusPercent ?? 0;
