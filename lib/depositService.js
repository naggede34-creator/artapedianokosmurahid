...
//   1. menanyakan status deposit ke provider (RuangOTP / Pakasir / RumahOTP), dan
...
import { getRuangOtpDepositStatus, isRuangOtpDeposit } from "@/lib/ruangotp";
...
export async function fetchProviderStatus(deposit) {
  try {
    if (isRuangOtpDeposit(deposit.provider)) {
      const data = await getRuangOtpDepositStatus(deposit.provider, deposit.providerRef || deposit.orderId);
      return data.status;
    }
...
export async function creditDeposit(deposit) {
  ...
  await logBalance({
    ...
    title: `Deposit ${providerName(deposit.provider)}`,
    ref: deposit.orderId
  });
