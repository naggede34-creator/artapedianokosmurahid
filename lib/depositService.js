...
//   1. menanyakan status deposit ke provider (Simuru / Pakasir / RumahOTP / VirtuSIM), dan
...
import { getVirtusimDepositStatus } from "@/lib/virtusim";
...
export async function fetchProviderStatus(deposit) {
  try {
    ...
    if (deposit.provider === "virtusim") {
      return await getVirtusimDepositStatus(deposit.providerRef || deposit.orderId);
    }
...
export async function creditDeposit(deposit) {
  ...
  await logBalance({
    ...
    title: `Deposit ${deposit.provider === "simuru" ? "QRIS Simuru" : deposit.provider === "rumahotp" ? "QRIS RumahOTP" : deposit.provider === "virtusim" ? "QRIS VirtuSIM" : "QRIS Pakasir"}`,
    ref: deposit.orderId
  });
