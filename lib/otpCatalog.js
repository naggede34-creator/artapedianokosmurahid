...
import { getRuangOtpServices, getRuangOtpCountries, isRuangOtpServer } from "@/lib/ruangotp";
...
export async function listServices(server = "rumahotp") {
  ...
  if (isRuangOtpServer(server)) {
    const list = await getRuangOtpServices(server);
    return sortServices(
      list.map((s) => ({ service_code: s.code, service_name: s.name, service_img: null, server }))
    );
  }
...
export async function listCountries(settings, server, serviceId) {
  ...
  if (isRuangOtpServer(server)) {
    const rows = await getRuangOtpCountries(server, serviceId);
    return rows
      .map((c) => ({
        ...
        pricelist: c.pricelist
          ...
          .map((p, idx) => ({
            // `key` dari klien RuangOTP sudah cukup untuk order — disimpan apa
            // adanya supaya alur order tidak perlu tahu versi API-nya.
            provider_id: p.key,
            provider_name: p.serverName || (idx === 0 ? "Paket Termurah" : `Paket ${idx + 1}`),
            price: p.price,
