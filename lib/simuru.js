...
async function resolveApiKey() {
  try {
    ...
    const { simuru } = await getApiKeys();
    return simuru || (process.env.SIMURU_APIKEY || "").trim();
  } catch {
    return (process.env.SIMURU_APIKEY || "").trim();
  }
}

export async function simuruConfigured() {
...
async function simuruRequest(path, params = {}, { method = "GET", timeout = 20000 } = {}) {
  const key = await resolveApiKey();
  if (!key) throw new SimuruError("API key Simuru belum diisi. Isi di Dashboard Admin → Pengaturan → API Keys.", 500);
