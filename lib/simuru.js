...
export async function getSimuruOtpServices() {
  const body = await simuruRequest("/services");
  return Array.isArray(body.data) ? body.data : [];
}
...
export async function createSimuruOtpOrder({ serviceId, countryId, operator = "any" }) {
...
// Status OTP Simuru: "active" = menunggu/pending, "expired"/"canceled"/"completed" = terminal
export const OTP_SIMURU_TERMINAL = ["expired", "canceled", "cancelled", "completed", "done"];

export function normalizeSimuruOtpStatus(raw) {
