  ...
  if (res.status >= 400 || !body || typeof body !== "object" || body.success === false) {
    ...
    if (!msg) {
      ...
      if (res.status === 401) msg = "API key Simuru tidak valid.";
        ...
        // Halaman challenge Cloudflare, bukan jawaban Simuru. Pembeli tidak perlu
        // tahu detail teknisnya — itu sudah tercatat di log & Diagnosa admin.
        blockedByCdn = true;
        msg = "Server OTP Fast sedang tidak bisa dihubungi. Silakan pakai server/metode lain dulu.";
    ...
    const e = new SimuruError(msg, res.status, body);
    e.blockedByCdn = blockedByCdn;
    throw e;
  }
  return body;
}
...
export async function diagnoseSimuru() {
  ...
  // "Just a moment..." = Managed Challenge Cloudflare: wajib menjalankan JavaScript,
  // jadi tidak bisa dilewati dari sisi server. Hanya pemilik simuru.com yang bisa
  // mengecualikan /api/* dari challenge itu.
  const challenged = blocked.some((c) => /just a moment|cf-browser-verification|challenge-platform/i.test(c.bodySnippet || ""));
  return {
    configured: true,
    ...
    verdict: blocked.length === 0
      ...
      : challenged
      ? "Cloudflare milik simuru.com menghadang dengan Managed Challenge (\"Just a moment...\"). Challenge ini wajib menjalankan JavaScript, jadi tidak bisa dilewati dari server. Minta admin Simuru mengecualikan /api/* dari Bot Fight Mode / Managed Challenge, sertakan cf-ray di bawah."
      : byWaf
