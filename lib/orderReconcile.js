// Logika inti pengecekan status pesanan OTP ke provider (RumahOTP / Simuru) + efek sampingnya
...
import { checkSimuruOtpStatus, OTP_SIMURU_TERMINAL } from "@/lib/simuru";
...
export async function reconcileOtpOrder(order) {
  ...
  if (order.server === "simuru") {
    const raw = await checkSimuruOtpStatus(order.orderId);
    data = raw || {};
    // Simuru mengembalikan sms_data sebagai array, ekstrak teks SMS pertama jika ada.
    if (!data.sms && Array.isArray(data.sms_data) && data.sms_data.length > 0) {
      data = { ...data, sms: data.sms_data[0]?.text || "" };
    }
    // Normalisasi nama field agar extractCode bisa menemukan kodenya.
    if (data.sms_code) data = { ...data, otp_code: data.sms_code };
  } else {
  ...
  if (newOtpCode) {
    ...
    const terminalList = order.server === "simuru" ? OTP_SIMURU_TERMINAL : KNOWN_TERMINAL;
