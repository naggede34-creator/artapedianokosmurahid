...
export async function reconcileOtpOrder(order) {
  ...
  if (otpJustArrived) {
    ...
    // Salinan ke channel, lalu dipin. TANPA kode OTP-nya: kode itu milik
    // pembeli yang membayarnya, dan siapa pun yang membacanya lebih dulu bisa
    // memakainya. Yang dibutuhkan channel cuma bukti bahwa pesanannya sampai.
    sendTelegramChannelNotif(
      otpReceivedPublicNotif({
        serviceName: order.serviceName,
        countryName: order.countryName,
