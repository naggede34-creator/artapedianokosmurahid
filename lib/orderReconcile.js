...
import { sendTelegramNotif, sendTelegramPhoto, otpReceivedNotif, otpAutoRefundNotif } from "@/lib/telegram";
import { generateReceiptPng, otpReceivedParams } from "@/lib/receiptImage";
...
export async function reconcileOtpOrder(order) {
  ...
  if (otpJustArrived) {
    ...
    generateReceiptPng(otpReceivedParams({
      orderId: order.orderId,
      token: order.token,
      serviceName: order.serviceName,
      countryName: order.countryName,
      ...
      otpCode: newOtpCode,
      waited
    })).then((png) => sendTelegramPhoto(png, receivedText.slice(0, 800))).catch((err) => console.error("[receipt/otp-received]", err?.message || err));
  }
