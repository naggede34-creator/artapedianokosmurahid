// Logika inti pengecekan status pesanan OTP ke provider (RumahOTP / RuangOTP / dibanana) + efek sampingnya
...
import { getRuangOtpStatus, isRuangOtpServer, RUANGOTP_TERMINAL } from "@/lib/ruangotp";
...
export async function reconcileOtpOrder(order) {
  ...
  const fromRuangOtp = isRuangOtpServer(order.server);
  ...
  } else if (fromRuangOtp) {
    const r = await getRuangOtpStatus(order.server, order.orderId);
  ...
  if (newOtpCode) {
    ...
    const terminalList = fromDibanana ? DIBANANA_TERMINAL : fromRuangOtp ? RUANGOTP_TERMINAL : KNOWN_TERMINAL;
