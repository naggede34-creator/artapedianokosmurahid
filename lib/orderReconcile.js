// Logika inti pengecekan status pesanan OTP ke provider (RumahOTP / OTPMANIA) + efek sampingnya
...
import { getOtpmaniaStatus, isOtpmaniaServer, OTPMANIA_TERMINAL } from "@/lib/otpmania";
...
export async function reconcileOtpOrder(order) {
  ...
  const fromOtpmania = isOtpmaniaServer(order.server);
  ...
  } else if (fromOtpmania) {
    const r = await getOtpmaniaStatus(order.orderId);
  ...
  if (newOtpCode) {
    ...
    const terminalList = fromDibanana ? DIBANANA_TERMINAL : fromOtpmania ? OTPMANIA_TERMINAL : KNOWN_TERMINAL;
