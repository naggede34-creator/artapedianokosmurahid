// Logika inti pengecekan status pesanan OTP ke provider (RumahOTP / VirtuSIM) + efek sampingnya
...
import { getVirtusimOrder, normalizeVirtusimOrderStatus, cancelVirtusimOrder } from "@/lib/virtusim";
...
// Toleransi setelah waktu kedaluwarsa lokal sebelum pesanan VirtuSIM dianggap habis.
const VIRTUSIM_EXPIRY_GRACE_MS = 2 * 60 * 1000;

// Ambil status terbaru pesanan dari provider-nya, dalam bentuk seragam
// { status, otp_code, sms, ... } supaya sisa logika di bawah tidak peduli providernya.
// Pesanan lama tanpa field `provider` otomatis dianggap RumahOTP.
async function fetchProviderOrderData(order) {
  if (order.provider === "virtusim") {
    const ref = order.providerRef || String(order.orderId || "").replace(/^VS/, "");
    let vs = null;
    try {
      vs = await getVirtusimOrder(ref);
    } catch (err) {
      console.error(`[reconcile] VirtuSIM ${ref} gagal dicek:`, err?.message || err);
    }
    let status = vs ? normalizeVirtusimOrderStatus(vs.rawStatus) : "pending";
    const hasCode = vs?.sms ? extractCode({ sms: vs.sms }) : null;

    // VirtuSIM tidak selalu mengirim label kedaluwarsa yang jelas, jadi waktu lokal
    // juga dipakai. Nomor dilepas dulu di provider (best-effort) sebelum saldo kembali.
    if (!hasCode && status === "pending") {
      const exp = order.expiredAt ? new Date(order.expiredAt).getTime() : null;
      if (exp && Date.now() > exp + VIRTUSIM_EXPIRY_GRACE_MS) {
        await cancelVirtusimOrder(ref).catch(() => {});
        status = "expired";
      }
    }
    return { status, sms: vs?.sms || null };
  }
...
export async function reconcileOtpOrder(order) {
  ...
  const data = await fetchProviderOrderData(order);
