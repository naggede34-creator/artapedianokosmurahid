...
const DEFAULTS = {
  ...
  // API keys — bisa diisi dari Dashboard Admin tanpa perlu set env vars ulang.
  rumahOtpApiKey: process.env.RUMAHOTP_APIKEY || "",
  simuruApiKey: process.env.SIMURU_APIKEY || "",
  ...
  otpMarkupPercent: Number(process.env.OTP_MARKUP_PERCENT || 0),
  smmMarkupPercent: Number(process.env.SMM_MARKUP_PERCENT || 0),
};
...
export async function updateSettings(patch) {
  ...
  if (patch.rumahOtpApiKey !== undefined) allowed.rumahOtpApiKey = String(patch.rumahOtpApiKey).trim().slice(0, 200);
  if (patch.simuruApiKey !== undefined) allowed.simuruApiKey = String(patch.simuruApiKey).trim().slice(0, 200);
  if (patch.pakasirProject !== undefined) allowed.pakasirProject = String(patch.pakasirProject).trim().slice(0, 100);
  if (patch.pakasirApiKey !== undefined) allowed.pakasirApiKey = String(patch.pakasirApiKey).trim().slice(0, 200);
  if (patch.cronSecret !== undefined) allowed.cronSecret = String(patch.cronSecret).trim().slice(0, 100);
  if (patch.adminCode !== undefined && String(patch.adminCode).trim()) allowed.adminCode = String(patch.adminCode).trim().slice(0, 100);
  if (patch.referralBonusPercent !== undefined) allowed.referralBonusPercent = Math.max(0, num(patch.referralBonusPercent));
  if (patch.otpMarkupPercent !== undefined) allowed.otpMarkupPercent = Math.max(0, num(patch.otpMarkupPercent));
  if (patch.smmMarkupPercent !== undefined) allowed.smmMarkupPercent = Math.max(0, num(patch.smmMarkupPercent));
