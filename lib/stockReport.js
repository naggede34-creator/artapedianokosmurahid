...
import { getWarungNokosCountries, isWarungNokosServer, warungNokosConfigured } from "@/lib/warungnokos";
...
async function warungNokosRow(settings, service, serverId) {
  if (!warungNokosConfigured()) return null;
  const markup = markupFn(settings, serverId);
  const rows = await getWarungNokosCountries(serverId, service);
    ...
    for (const srv of activeServers) {
      jobs.push(
        (async () => {
          ...
          if (isWarungNokosServer(srv.key)) return warungNokosRow(settings, service, srv.key);
          return null;
        })()
