...
import { calonBase, jelaskanGagal } from "@/lib/webhookBase";
...
/**
 * Secret webhook bot ini. Dibaca dari database, bukan dibuat baru tiap kali:
 * secret yang berubah sesudah webhook terpasang membuat SEMUA update botnya
 * ditolak 401, dan tidak ada error yang muncul di mana pun.
 */
async function secretUntuk(token, botId) {
  const col = await botsCol();
  const b = await col.findOne({ botId: String(botId) });
  if (b?.webhookSecret) return b.webhookSecret;
  const secret = buatSecret();
  await col.updateOne({ botId: String(botId) }, { $set: { webhookSecret: secret, token } }, { upsert: true });
  return secret;
}
...
async function setWebhookSekali(token, url, secret) {
...
/**
 * Memasang webhook dengan mencoba beberapa alamat berurutan.
 *
 * Satu alamat saja berarti satu huruf salah di Pengaturan mematikan seluruh
 * fitur, dan admin cuma melihat "Failed to resolve host" tanpa tahu alamat
 * apa yang dicoba. Calon terakhir selalu alamat deployment Vercel, yang
 * dibuat Vercel sendiri dan karenanya pasti bisa di-DNS.
 */
export async function pasangWebhook(token, botId, calon) {
  const daftar = Array.isArray(calon) ? calon : [];
  if (!daftar.length) {
    return {
      ok: false,
      alasan: "Tidak ada alamat situs yang bisa dipakai. Isi Site URL di tab Pengaturan dengan alamat situs ini.",
      percobaan: []
    };
  }

  const secret = await secretUntuk(token, botId);
  const percobaan = [];

  for (const { base, asal } of daftar) {
    const url = alamatWebhook(base, botId);
    const r = await setWebhookSekali(token, url, secret);
    if (r?.ok) {
      return {
        ok: true,
        url,
        asal,
        percobaan,
        // Kalau yang berhasil BUKAN calon pertama, alamat di Pengaturan salah
        // dan bot pertama memakai sumber yang sama — jadi ia ikut rusak.
        // Admin harus tahu, bukan cuma senang webhook-nya hijau.
        peringatanSumber:
          daftar[0].base !== base
            ? `Webhook terpasang lewat ${asal}, bukan lewat ${daftar[0].asal} (${daftar[0].base}) yang ternyata tidak bisa dihubungi Telegram. Perbaiki Site URL di Pengaturan — bot pertama memakai sumber yang sama dan kemungkinan ikut mati.`
            : ""
      };
    }
    percobaan.push({ url, asal, alasan: jelaskanGagal(r?.description, url) });
  }

  return {
    ok: false,
    alasan: percobaan[0]?.alasan || "setWebhook gagal.",
    percobaan
  };
}
 ...
 */
export async function tambahBot({ token, calon, ditambahOleh }) {
  ...
  const webhook = await pasangWebhook(bersih, cek.botId, calon);
  ...
  await col.updateOne(
    { botId: cek.botId },
    {
      $set: {
        ...
        // Yang disimpan adalah kalimat yang MENYEBUT PERBAIKANNYA, bukan
        // pesan mentah Telegram. "Failed to resolve host" tidak memberi tahu
        // siapa pun apa yang harus diubah.
        webhookPesan: webhook.ok ? webhook.peringatanSumber || "" : webhook.alasan || "",
        webhookPercobaan: (webhook.percobaan || []).map((p) => ({ url: p.url, asal: p.asal, alasan: p.alasan }))
      }
    }
  );
...
/**
 * Menyimpan hasil pemasangan webhook ke dokumen botnya.
 *
 * Dipanggil juga dari aksi "pasang ulang" dan "nyalakan": tanpa ini, lencana
 * WEBHOOK OK / BELUM di daftar tetap menunjukkan keadaan LAMA sesudah admin
 * memperbaiki Site URL-nya — dan ia akan mengira perbaikannya tidak berhasil.
 */
export async function botsUpdateWebhook(botId, hasil) {
  const col = await botsCol();
  await col.updateOne(
    { botId: String(botId) },
    {
      $set: {
        webhookOk: !!hasil?.ok,
        webhookUrl: hasil?.url || "",
        webhookPesan: hasil?.ok ? hasil?.peringatanSumber || "" : hasil?.alasan || "",
        webhookPercobaan: (hasil?.percobaan || []).map((p) => ({ url: p.url, asal: p.asal, alasan: p.alasan }))
      }
    }
  );
}
...
export async function daftarBot() {
  ...
  return rows.map((b) => ({
    ...
    // Alamat yang benar-benar dipakai ikut ditampilkan. Inilah yang selama
    // ini tidak kelihatan, dan karenanya kegagalan DNS jadi misteri.
    webhookUrl: b.webhookUrl || "",
    webhookPercobaan: Array.isArray(b.webhookPercobaan) ? b.webhookPercobaan : [],
